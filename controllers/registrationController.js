...
router.post("/register/:id", (req, res) => {
  ...
  if (req.body.birth_month && req.body.birth_day && req.body.birth_year) {
    birthdate = new Date(
      `${req.body.birth_month} ${req.body.birth_day}, ${req.body.birth_year}`
    );
  }

  const email = req.body.minorEmail
    ? req.body.adultEmail + ", " + req.body.minorEmail
    : req.body.adultEmail;
  const eContact1Name =
    req.body.emergency_contact_1_first_name ||
    req.body.emergency_contact_1_last_name
      ? req.body.emergency_contact_1_first_name +
        " " +
        req.body.emergency_contact_1_last_name
      : "";
  const eContact2Name =
    req.body.emergency_contact_2_first_name ||
    req.body.emergency_contact_2_last_name
      ? req.body.emergency_contact_2_first_name +
        " " +
        req.body.emergency_contact_2_last_name
      : "";
  const buddyName =
    req.body.buddy_first_name || req.body.buddy_last_name
      ? req.body.buddy_first_name + " " + req.body.buddy_last_name
      : "";
  ...
  const registrationObj = {
    ...
    contact_for_volunteer:
      req.body.contact_for_volunteer === "yes" ? trueString : falseString,

    EventId: 1, // a hook for later when there are more events
    UserId: req.session.user.id,
  };

  db.Registration.create(registrationObj).then((newRegistration) => {
    ...
    if (selectedRole === "volunteer" || selectedRole === "childVolunteer") {
      ...
      const volunteerObj = {
        ...
        camp_unit: req.body.grade_in_school
          ? req.body.grade_in_school
          : selectedRole,
      ...
      db.VolunteerInfo.create(volunteerObj).then((newVolunteerInfo) => {
        console.log(newVolunteerInfo);

        res.redirect(
          `/health?role=${selectedRole}&regId=${newRegistration.id}`
        );
      });
    } else {
    ...
    db.Registration.update(
      {
        ...
        emergency_contact_1_relationship:
          req.body.emergency_contact_1_relationship,
        emergency_contact_2_name: req.body.emergency_contact_2_name,
        emergency_contact_2_phone: req.body.emergency_contact_2_phone,
        emergency_contact_2_relationship:
          req.body.emergency_contact_2_relationship,
